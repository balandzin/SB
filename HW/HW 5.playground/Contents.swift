...
func forEvenParity(number: Int) -> Bool {
    number % 2 == 0
}

print(forEvenParity(number: number))
/*:
...
*/
func dividedIntoThree(number: Int) -> Bool {
    number % 3 == 0
}

print(dividedIntoThree(number: number))
/*:
...
func getArray(from lowerLimit: Int, until upperLimit: Int) -> Set<Int> {
    ...
    print("Множество размером \(count) элементов: \(numbers)")
    return numbers
}
 ...
 */
let integers = [1, 2, 3, 4, 5, 6, 7, 8, 8, 9, 10]

func forEvenParity(numbers: [Int]) -> [Int] {
    var unevenNumbers: [Int] = []
    
    for number in numbers {
        if !forEvenParity(number: number) {
...
forEvenParity(numbers: integers)
